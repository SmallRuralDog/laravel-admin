        ...
        return Form::make(SystemMenu::query(), function (Form $form) {
            ...
            $form->customLayout([
                ...
                $form->item('icon', __("admin::admin.icon"))
                    ->useFormItem(Select::make()->options($this->getIconList())->menuTpl('<i class="${value} mr-2"></i>${label}'))
                    ->required(true)
                    ->visibleOn('type=="dir"')
                    ->description("<a target='_blank' href='https://fontawesome.com/search?o=r&ic=free&s=solid&ip=classic'>Font Awesome</a>")
  ...
  "brands": [
    ...
    "fa-42-group"
        ...
        foreach ($solid as $item) {
            $list['fa-solid ' . $item] = 'fa-solid ' . $item;
        }
        foreach ($regular as $item) {
            $list['fa-regular ' . $item] = 'fa-regular ' . $item;
        }
        foreach ($brands as $item) {
            $list['fa-brands ' . $item] = 'fa-brands ' . $item;
        }
        return $list;
    }
}
